---
title: 'How to Migrate from AWS App Runner to Amazon ECS'
author_id: FivexL
date: 2026-07-09
description: "AWS is deprecating App Runner. Here's why we recommend skipping ECS Express Mode and going straight to ECS on Fargate, plus the security and networking gaps AWS's migration guide leaves out."
author: FivexL
...
App Runner was AWS's opinionated "give us a container or a repo, we handle the rest" service. The trade-off was always the same: fast setup in exchange for limited control over networking, observability, and CI/CD. With App Runner going away, teams have to make choices they never had to make before.

The migration paths on the table:

- **ECS Express Mode**: opinionated, fast to set up, closer to the App Runner experience.
- **ECS on Fargate (standard)**: more configuration up front, full control, easier to run under HIPAA or SOC 2 expectations.
- **Other platforms**: AWS Lambda for event-driven workloads, or third-party PaaS options.

For anyone running a production workload that will still be around in a year, we think the second option is the one that saves you a second migration later.
...
There is also a networking difference worth understanding. App Runner used a public-facing Network Load Balancer (NLB) at Layer 4 paired with an internal Layer 7 request router. ECS Express Mode only supports ALB <sup>[5]</sup>, which operates at Layer 7. If you need NLB behavior (TCP-level routing, static IP addresses, ultra-low latency), you have to move to standard ECS.

With both ALB and NLB, you can attach multiple SSL certificates to a single listener (a default certificate plus additional ones via SNI), and with ALB specifically you can define listener rules that route traffic based on host header, URL path, query string, or HTTP method.

That flexibility is meaningful once you outgrow a single-service setup.

### Observability requires a real plan now

App Runner shipped with AWS X-Ray integration out of the box. With ECS, tracing requires a sidecar container running the X-Ray daemon or an OpenTelemetry collector.
...
Once you are on ECS with Fargate, you qualify for AWS Compute Savings Plans <sup>[8]</sup>, which offer meaningful discounts on your compute bill in exchange for a one- or three-year commitment. For Fargate workloads with predictable baseline usage, this is usually the largest cost lever available. App Runner had no equivalent program.

## Our recommendation

If you are already on App Runner in production, we would plan the migration in this order:

1. Stand up a VPC that meets your security baseline (private subnets, VPC endpoints, flow logs).
2. Move to standard ECS on Fargate rather than Express Mode, so dev, staging, and production all use the same runtime.
3. Build the CI/CD pipeline once, in code, so every environment deploys the same way.
4. Wire up observability with OpenTelemetry from day one.
5. Once workload is stable, evaluate Compute Savings Plans.
...
If you would rather not build all of this from scratch, our [ECS Blueprint](https://fivexl.io/ecs-blueprint/) is a production-ready application platform for teams that need to run containers reliably on ECS and Fargate, ship features quickly, and stay aligned with HIPAA or SOC 2 expectations as they grow.
...
Built specifically for regulated SaaS startups, the Blueprint replaces the fragile quick fixes (single environment, ad-hoc deploys, missing controls) that tend to break under real usage and stall security reviews.
...
9. [AWS App Runner service architecture](https://docs.aws.amazon.com/apprunner/latest/dg/architecture.html)
...
14. [Saving money with EKS Fargate and Compute Savings Plans (AWS Blog)](https://aws.amazon.com/jp/blogs/containers/saving-money-pod-at-time-with-eks-fargate-and-aws-compute-savings-plans/)
